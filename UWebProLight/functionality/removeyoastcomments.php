...
class RYSC {
	...
	public function dash_widget() {
		...
		wp_add_dashboard_widget( 'dashboard_widget', 'UWebPros Remove Yoast SEO Comments <span></span><script>/*
		
	  var str = document.querySelector(".wrap").innerHTML; 
  var res = str.replace("<h1>Dashboard</h1>", "<h1>UWebPro 2.0</h1>");
...
*/
	...
	public function dash_widget_content() {
		...
		if($this->operating_status() == 1) {
			...
			$content = '<p>Version ' . WPSEO_VERSION . ' of Yoast SEO is fully supported by UWebPro ' . $this->version . '. Frontend Comments have been removed.</p>';

		} elseif($this->operating_status() == 2) {

			$status = '<span style="color:#FF8000;font-weight:bold">Partially supported</span>';

			$content = '<p>Version ' . WPSEO_VERSION . ' of Yoast SEO is not properly supported by UWebPro ' . $this->version . '. Some functions are not working. A backup solution has been enabled to keep the HTML comments removed.</p>';

		} elseif($this->operating_status() == 3) {

			$status = '<span style="color:#DF0101;font-weight:bold">Not supported</span>';

			$content = '<p>Version ' . WPSEO_VERSION . ' of Yoast SEO is not supported by UWebPro ' . $this->version . '. A backup solution has been enabled to keep the HTML comments removed.</p>';
